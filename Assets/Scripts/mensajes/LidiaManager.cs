...
public class LidiaManager : MonoBehaviour
{
    ...
    private readonly float esperaLidiaSegundos = 2f;
    ...
    public void Seleccionar()
    {
        ...
        if (nombre == "1OpcionSelector(8)")
        {
            ElegirOpcion(1);
        }
        else if (nombre == "2OpcionSelector(8)")
        {
            ElegirOpcion(2);
        }
        else if (nombre == "3OpcionSelector(8)")
        {
            ElegirOpcion(3);
        }
        else if (nombre == "BotonAtras(8)")
        {
            CerrarModoTeclado();
            mensajesManager.VolverAlMenu();
        }
        else if (nombre == "BotonInicio(8)")
        {
            CerrarModoTeclado();
    ...
    {
        string mensaje = NormalizarMensaje(mensajeUsuario);

        if (nivelActual == 1)
        {
            if (mensaje == NormalizarMensaje("Sí")) return 1;
            if (mensaje == NormalizarMensaje("No")) return 2;
            if (mensaje == NormalizarMensaje("Olé!")) return 3;
        }
        else if (nivelActual == 2)
        {
            ...
            {
                if (mensaje == NormalizarMensaje("Sí")) return 1;
                if (mensaje == NormalizarMensaje("No mucho")) return 2;
                if (mensaje == NormalizarMensaje("Un poco Olé!")) return 3;
            }
            else if (opcionNivel1Elegida == 2)
            {
                if (mensaje == NormalizarMensaje("Sí")) return 1;
                if (mensaje == NormalizarMensaje("No")) return 2;
                if (mensaje == NormalizarMensaje("Obvio")) return 3;
            }
            else if (opcionNivel1Elegida == 3)
            {
                if (mensaje == NormalizarMensaje("Sí")) return 1;
                if (mensaje == NormalizarMensaje("Regular")) return 2;
                if (mensaje == NormalizarMensaje("Bien")) return 3;
            }
        }
    ...
    string NormalizarMensaje(string texto)
    {
        texto = texto.ToLower();
    ...
    IEnumerator MostrarRespuestaNoDisponible(string mensajeUsuario)
    {
        ...
        string mensajeLidia = "Respuesta no disponible";
        ...
        if (nivelActual == 1)
        {
            ...
            yield return new WaitForSeconds(esperaLidiaSegundos);

            if (texto2Recibido != null) texto2Recibido.text = mensajeLidia;
            if (recibido2 != null) recibido2.SetActive(true);

            minutosExtra++;
            PonerHora(hora2Recibido, minutosExtra);

            mensajesManager.ActualizarChat(7, mensajeLidia, hora2Recibido.text, false);
        ...
        else if (nivelActual == 2)
        {
            ...
            if (texto3Recibido != null) texto3Recibido.text = mensajeLidia;
            if (recibido3 != null) recibido3.SetActive(true);

            minutosExtra++;
            PonerHora(hora3Recibido, minutosExtra);

            mensajesManager.ActualizarChat(7, mensajeLidia, hora3Recibido.text, false);
    ...
    IEnumerator MostrarSecuencia(int opcion)
    {
        ...
        if (nivelActual == 1)
        {
            ...
            {
                mensajeUsuario = "Sí";
                mensajeLidia = "Yo tambien lo noto, ¿se me ve mas suelta?";
            }
            else if (opcion == 2)
            {
                mensajeUsuario = "No";
                mensajeLidia = "Bueno, seguire intentando, ¿se nota algo al menos?";
            }
            else if (opcion == 3)
            {
                mensajeUsuario = "Olé!";
                mensajeLidia = "Jajaja, me anima, ¿tu como lo ves?";
            }
        ...
        else if (nivelActual == 2)
        {
            ...
            if (opcionNivel1Elegida == 1)
            {
                ...
                {
                    mensajeUsuario = "Sí";
                    mensajeLidia = "Poco a poco voy avanzando.";
                }
                else if (opcion == 2)
                {
                    mensajeUsuario = "No mucho";
                    mensajeLidia = "Bueno, aun me queda.";
                }
                else
                {
                    mensajeUsuario = "Un poco Olé!";
                    mensajeLidia = "Jajaja, gracias.";
                }
            }
            else if (opcionNivel1Elegida == 2)
            {
                ...
                {
                    mensajeUsuario = "Sí";
                    mensajeLidia = "Eso intento.";
                }
                else if (opcion == 2)
                {
                    mensajeUsuario = "No";
                    mensajeLidia = "Hay dias mejores.";
                }
                else
                {
                    mensajeUsuario = "Obvio";
                    mensajeLidia = "Jajaja.";
                }
            }
            else if (opcionNivel1Elegida == 3)
            {
                ...
                {
                    mensajeUsuario = "Sí";
                    mensajeLidia = "Gracias.";
                }
                else if (opcion == 2)
                {
                    mensajeUsuario = "Regular";
                    mensajeLidia = "Bueno, sigo poco a poco.";
                }
                else
                {
                    mensajeUsuario = "Bien";
                    mensajeLidia = "Me alegra oir eso.";
                }
            }
    ...
    void ReiniciarConversacion()
    {
        ...
        if (texto1Recibido != null) texto1Recibido.text = "Ya he terminado los ejercicios de hoy, ¿crees que voy mejor?";
        if (hora1Recibido != null) hora1Recibido.text = "15:25";
    ...
    {
        if (textoOpcion1 != null) textoOpcion1.text = "Sí";
        if (textoOpcion2 != null) textoOpcion2.text = "No";
        if (textoOpcion3 != null) textoOpcion3.text = "Olé!";
    }
    ...
    void MostrarOpcionesNivel2()
    {
        ...
        {
            if (textoOpcion1 != null) textoOpcion1.text = "Sí";
            if (textoOpcion2 != null) textoOpcion2.text = "No mucho";
            if (textoOpcion3 != null) textoOpcion3.text = "Un poco Olé!";
        }
        else if (opcionNivel1Elegida == 2)
        {
            if (textoOpcion1 != null) textoOpcion1.text = "Sí";
            if (textoOpcion2 != null) textoOpcion2.text = "No";
            if (textoOpcion3 != null) textoOpcion3.text = "Obvio";
        }
        else if (opcionNivel1Elegida == 3)
        {
            if (textoOpcion1 != null) textoOpcion1.text = "Sí";
            if (textoOpcion2 != null) textoOpcion2.text = "Regular";
            if (textoOpcion3 != null) textoOpcion3.text = "Bien";
        }
    }
    ...
    void PonerHora(TMP_Text textoHora, int minutosSumar)
    {
        ...
        System.DateTime baseHora = System.DateTime.Today.AddHours(15).AddMinutes(25);
